...
class SignupWindow(QWidget):
    """Allow a new user to create an account through the GUI."""

    def __init__(
        self,
        client_socket: ClientSocket,
        on_login_requested: Callable[[], None],
        on_authenticated: Callable[[], None],
    ) -> None:
        """Create the signup window.

        :param client_socket: Connected client socket used for signup requests.
        :param on_login_requested: Callback used to switch back to login.
        :param on_authenticated: Callback used after successful signup.
        """
        super().__init__()
    ...
    def _build_ui(self) -> None:
        """Build the signup form widgets and layout."""
        self.setWindowTitle("Secure File Manager - Signup")
        self.resize(540, 520)
        ...
        subtitle_label = QLabel("Your files will be stored in your private server folder.")
        ...
        switch_layout.addWidget(self.login_button)
    ...
    def _connect_signals(self) -> None:
        ...
        self.login_button.clicked.connect(self.on_login_requested)
        self.password_input.returnPressed.connect(self._handle_signup) # if enter is pressed on password input field, handle_sigunp will happen.

    def _handle_signup(self) -> None:
        """Validate the form, send the signup request, and handle the result."""
        username = self.username_input.text().strip()
        email = self.email_input.text().strip()
        password = self.password_input.text()
        validation_error = self._validate_form(username, email, password)
        if validation_error:
            QMessageBox.warning(self, "Signup details needed", validation_error)
            return

        self._set_busy(True)
        try:
            result = self.client_socket.signup(
                username=username,
                email=email,
                password=password,
            )
        ...
        QMessageBox.warning(self, "Signup failed", result.message)

    def _validate_form(self, username: str, email: str, password: str) -> str | None:
        """Validate signup form fields before contacting the server.
    ...
    def _set_busy(self, is_busy: bool) -> None:
        ...
        """
        self.username_input.setEnabled(not is_busy)
